...
export function ServiceOfferingsPage({ services }: ServiceOfferingsPageProps) {
  ...
  const providers = useMemo(
    () => ['All', ...Array.from(new Set(services.map((service) => service.cloudProvider))).sort()],
    [services],
  );
  const categories = useMemo(
    () => ['All', ...Array.from(new Set(services.map((service) => service.serviceCategory))).sort()],
    [services],
  );
  ...
  return (
    <div>
      ...
      <FilterBar search={search} onSearchChange={setSearch} providers={providers} categories={categories} selectedProviders={selectedProviders} onProvidersChange={setSelectedProviders} selectedCategories={selectedCategories} onCategoriesChange={setSelectedCategories} selectedTrmStatuses={selectedTrmStatuses} onTrmStatusesChange={setSelectedTrmStatuses} totalCount={services.length} filteredCount={filtered.length} onClearAll={clearAll} />
